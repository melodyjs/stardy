...
{
    ::STRINGS[2] = uString::Const("/usr/local/share/uno/Packages/Fuse.Testing/1.2.1/$.uno");
...
void TestRootPanel::RunTest()
{
...

    {
        ...
        catch (const uThrowable& __t)
        {
            ...
            if (testFailedException != NULL)
                ::g::Uno::Testing::Assert::Fail(uPtr(testFailedException)->Message(), ::STRINGS[2/*"/usr/local/...*/], 184, ::STRINGS[3/*"RunTest"*/]);
            else
                throw __t;
        }
    }
    ...
    for (::g::Uno::Collections::List__Enumerator<uStrong< ::g::Fuse::Diagnostic*> > enum2 = (::g::Uno::Collections::List__GetEnumerator_fn(uPtr(_errors), &ret7), ret7); enum2.MoveNext(::TYPES[9/*Uno.Collections.List<Fuse.Diagnostic>.Enumerator*/]); )
    {
        ...
        ::g::Uno::Testing::Assert::Fail(uPtr(e1)->ToString(), ::STRINGS[2/*"/usr/local/...*/], 196, ::STRINGS[3/*"RunTest"*/]);
    }
